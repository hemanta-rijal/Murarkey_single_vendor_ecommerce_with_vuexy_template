...
class Conversation extends Eloquent
{
     ...
     * @return     array
     */
    public function basicUserConversations($userId)
    {
    ...
    {
        $messageCount = MessageNotification::select(\DB::raw('COUNT(DISTINCT(conversation_id)) as message_count'))->where('user_id', $userId)
            ->where('is_seen', 0)
            ->first();

        return $messageCount ? $messageCount->message_count : 0;
    }
}
