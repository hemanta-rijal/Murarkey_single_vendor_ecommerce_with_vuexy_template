...
function verifyKhaltiPayment(payload, amt, targetBtn) {
  ...
  // ajax call
  $.post("app/Cart/verify.php", payload, function (resp) {
    debuginfo(
      "success",
      "Payment verification response : <br /><pre>" + resp + "</pre>"
    );
...
$(function () {
  $(".pay-khalti").on("click", function (e) {
    ...
    var amt = jQuery("#totalAmount").text();
    ...
    var config = {
      ...
      eventHandler: {
        onSuccess(payload) {
          debuginfo(
            "success",
            "Success callback received. <br />" + JSON.stringify(payload)
          );
        ...
        onError(error) {
          ...
          debuginfo(
            "danger",
            "Error callback received. <br />" + JSON.stringify(error)
          );
