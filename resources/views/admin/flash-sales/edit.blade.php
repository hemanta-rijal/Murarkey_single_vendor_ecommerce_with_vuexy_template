	...
	<script>
	 ...
	 function generateAddProductTemplate(product, index) {
	  ...
	  return '<tr id="product-' + product.id + '">' +
	   ...
	   '<td><select  id="discount_type_' + product.id + '" name="products[' + index + '][discount_type]"  class="form-control" style="width: auto;" onchange="updateDiscountedPrice(' + product.id + ',' +
	   product.price + ')"><option value="price">Price</option><option value="percentage">%</option></select></td>' +

	   '<td><input type="number" id="discount_' + product.id + '" name="products[' + index + '][discount]" class="form-control" onchange="updateDiscountedPrice(' + product.id + ',' + product.price +
	   ')" ></td>' +
														...
														<div class="row">
															<div class="col-12">
																<div class="form-group">
																	<label for="name-vertical">Flash Sale Title<span class="text-danger">*</span></label>
																	<input type="text" class="form-control" name="title" placeholder="Flash Sale Title" value="{{ $flashSale->title }}" required>
																</div>
															</div>
															<div class="col-12">
																<div class="form-group">
																	<label for="start_time-vertical">Start Time<span class="text-danger">*</span></label>
																	<input type="text" id="start_time-vertical" class="form-control pickadate" name="start_time" placeholder="Flash Sale Start Time" value="{{ $flashSale->start_time->format('d M, Y') }}"
																		required>
																</div>
															</div>
															<div class="col-12">
																<div class="form-group">
																	<label for="wend_timet-vertical">End Time<span class="text-danger">*</span></label>
																	<input type="text" id="end_time-vertical" class="form-control pickadate" name="end_time" placeholder="Flash Sale End Time" value="{{ $flashSale->end_time->format('d M, Y') }}"
																		required>
																</div>
															</div>
															<div class="col-12">
																	...
																	<tbody id="featured-products-tbody">
																		...
																		@foreach ($flashSale->items as $item)
																			<tr id="old-item-{{ $item->id }}">
																				<td><a href="{{ route('products.show', $item->product->slug) }}" target="_blank">{{ $item->product->name }}</a></td>
																				<td><select id="discount_type_{{ $item->id }}" name="products[{{ $loop->index }}][discount_type]" value="{{ $item->discount_type }}" class="form-control"
																						style="width: auto;" onchange="updateDiscountedPrice('{{ $item->id }}', '{{ $item->product->price }}')">
																				...
																				<td><input id="discount_{{ $item->id }}" type="number" name="products[{{ $loop->index }}][discount]" value="{{ $item->discount }}" class="form-control"
																						onchange="updateDiscountedPrice('{{ $item->id }}', '{{ $item->product->price }}')" /></td>

																				<td><input type="number" name="products[{{ $loop->index }}][actual_price]" value="{{ $item->actual_price }}" readonly="readonly" class="form-control"></td>

																				<td><input id="discounted_price_{{ $item->id }}" type="number" name="products[{{ $loop->index }}][discounted_price]" value="{{ $item->discounted_price }}"
																						readonly="readonly" class="form-control"></td>

																				<input type="hidden" name="products[{{ $loop->index }}][id]" value="{{ $item->id }}" class="form-control">
																				<td>
																					<button class="btn-sm btn-danger" onclick="removeProduct({{ $item->id }},{{ $item->product->id }})">Remove
																					</button>
																				</td>
																			</tr>
																		@endforeach
												...
												<div class="card-content">
													<div class="card-body">
														<div class="input-group input-group form-group">
															<input id="search-input-field " type="text" name="search" class="form-control no_border_radius" placeholder="Keyword" value="{{ request()->search }}"
																onkeypress="if(event.keyCode == 13) {getSearchResult()}">
															<span class="input-group-btn">
																<button class="btn btn-default" onclick="getSearchResult()"><i class="fa fa-search"></i></button>
															</span>
														</div>
