    ...
    <div class="db-sidebar">
        <div class="user">
            <div class="user-img-box">
                <img id="profilePicture" src="{{Auth::guard('web')->user()->profile_pic_url}}"
                     alt=" {{Auth::guard('web')->user()->name}}">
                <a href="#" class="overlay">
                    <i class="fa fa-pencil"></i>
                    Change picture
                </a>
                <form id="updateUserProfilePicture" action="{{route('user.upload-profile-pic')}}" method="post"
                      enctype="multipart/form-data">
                    @csrf
                    <input type="file" name="profile_pic" id="changePic" onchange="updateProfile(this.files[0])"
