                            ...
                            <div class="product-pic-zoom">
                                @if($product->images->count() > 0)
                                    <img class="product-big-img"
                                         src="{{resize_image_url($product->featured_image,'600X600')}}"
                                         alt="{{$product->slug}}"/>
                                @endif
                                <div class="zoom-icon">
                                ...
                                <div class="product-thumbs-track ps-slider owl-carousel">
                                    @foreach($product->images as $image)
                                        <div class="pt  {{ $loop->first ? 'active': ''}}"
                                             data-imgbigurl="{{resize_image_url($image->image,'200X200')}}">
                                            <img src="{{resize_image_url($image->image,'600X600')}}"
                                                 alt="{{$product->slug}}"/>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="col-lg-6">
                            <form id="option-choice-form">
                                ...
                                <div class="product-details">
                                    ...
                                    <a href="#" class="heart-icon btn btn-outline-danger mb-4 btn-block"
                                       onclick="addToWishlist({{$product->id}})" data-value="{{$product->id}}"><i
                                                class="icon_heart_alt"></i> save in Wishlist </a>
                                    <ul class="pd-tags">
                                        <li><span>Availability</span>:
                                            <b> {{$product->total_product_units >0 ? ' Stock Available' : 'Out Of Stock'}}</b>
                                        </li>
                                        <li><span>CATEGORIES</span>: <b><a
                                                        href="{{route('products.search','category='.$product->category->slug)}}"
                                                        style="color: blue;">{{$product->category->name}}</a></b></li>
                                        <li><span>Brand</span>: <b><a
                                                        href="{{route('products.search','brand='.$product->brand->slug)}}"
                                                        style="color: blue;"> {{$product->brand->name}}</a></b></li>
                                        <li><span>SKU</span>: <b>{{$product->sku}}</b></li>
                                        @if($product->rel_keywords->pluck('name')->first())
                                            <li><span>TAGS</span>:
                                                <b>{{$product->rel_keywords->pluck('name')->first()}}</b></li>
                                        @endif
                                    </ul>
                                </div>
                            </form>
                                                ...
                                                @foreach($product->reviews->take(5) as $review)
                                                    <div class="co-item">
                                                        <div class="avatar-pic">
                                                            <img src="{{$review->user->profile_pic_url}}"
                                                        ...
                                                        <div class="avatar-text">
                                                            ...
                                                            <h5>{{$review->user->name}}
                                                                <span>{{$review->formated_created_at}}</span></h5>
                                                            <div class="at-reply">
                                                                {{$review->comment}}
                                                ...
                                                <form action="{{route('user.reviews.store')}}" method="POST"
                                                    ...
                                                    <div class="personal-rating form-group mt-3 mb-4">
                                                        ...
                                                        <div class="product-rating give-stars mt-2">
                                                            <span data-value="1" class="user-rating"><i
                                                                        class="fa fa-star"></i></span>
                                                            <span data-value="2" class="user-rating"><i
                                                                        class="fa fa-star"></i></span>
                                                            <span data-value="3" class="user-rating"><i
                                                                        class="fa fa-star"></i></span>
                                                            <span data-value="4" class="user-rating"><i
                                                                        class="fa fa-star"></i></span>
                                                            <span data-value="5" class="user-rating"><i
                                                                        class="fa fa-star"></i></span>
                                                        </div>
                            ...
                            <div class="product-item">
                                <div class="pi-pic">
                                    <a href="{{ route('products.show', $recent_product->slug) }}">
                                        <img src="{{resize_image_url($recent_product->featured_image,'200X200')}}"
                                             alt="{{$recent_product->slug}}"/>
                                    </a>
                                    <div class="icon">
                                        <i class="icon_heart_alt"></i>
                                    </div>
                                    <ul>
                                        <li class="quick-view"><a
                                                    href="{{ route('products.show', $recent_product->slug) }}">Add to
                                                Card</a></li>
                                    </ul>
                                </div>
                                <div class="pi-text">
