                            ...
                            @foreach(get_root_categories()->take(12) as  $category)
                                <li class="list-group-item categories-item">
                                    <a href="/products/search?category={{ $category->slug }}" class="categories link">
                                        <img src="{{resize_image_url($category->image_path, '50X50')}}"
                                             alt="{{$category->name}}"
                                                    ...
                                                    @if($category->children->count() > 0)
                                                        <ul>
                                                            @foreach($subCategory->children as $subSubCategory)
                                                                <li>
                                                                    <a href="/products/search?category={{ $subSubCategory->slug }}"> {{ $subSubCategory->name }}</a>
                                                                </li>
                                                            @endforeach
                                                        </ul>
                                                    @endif
                                ...
                                <div class="image">
                                    <a href="{{ $slide->link ? $slide->link : '' }}">
                                        <img src="{{ map_storage_path_to_link($slide->image) }}"
                                             alt="{{ $slide->caption }}" style="height: auto; object-fit: cover;"/>
                                    </a>
                                </div>
