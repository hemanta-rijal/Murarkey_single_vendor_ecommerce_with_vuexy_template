                    ...
                    <div class="searchbox">
                        <form>
                            <div class="input-group input-group-sm">
                                <input type="text" name="Please Enter User's Name" class="form-control no_border_radius"
            ...
            <div class="row m-t-20">
                <div class="col-md-12">
                    <div class="pum_table_wrapper m-b-20" id="inbox_table_wrapper"
                         style="max-height: 680px;overflow-y: auto;border-right: 1px solid #e3e3e3;">
                        ...
                        <table class="table table-responsive m-b-0">
                            ...
                            @foreach($messages as $message)
                                <tr class="{{ $message->notification && $message->notification->status == 'read' ? 'already_read' : 'not_yet_read' }}
                                        ">
                                    <td>
                                        <a href="{{ route('user.message-center.single-system-news', $message->from) }}">{{ $message->sender->name }}</a>
                                    </td>
                                    <td>
                                        <a href="{{ route('user.message-center.single-system-news', $message->from) }}">{{ $message->subject }}</a>
                                    </td>
                                    <td>
                                        <a href="{{ route('user.message-center.single-system-news', $message->from) }}">{{ formatDateString($message->updated_at) }}</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
